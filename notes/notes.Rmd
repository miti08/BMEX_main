...
library(tidyr)
```
We will import the resulting csv file back into R and do some minimal cleaning. This code is pretty horrible but it will do. 
```{r}
...
```
We will tidy up the data to make analysis easier using the tidyverse packages. Tidying up here means that we basically pivot the table from a matrix to a list of observations (i.e. each row represent one sample, one cell_type, and its fraction)
``` {r}
...
So that is quite a mess. Let's zoom in on the top cell types by median fraction. 
